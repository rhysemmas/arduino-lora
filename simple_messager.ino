...
#define buffer_size 20 // Max message size: 251
...
void checkForMessages() {
  if (rf95.available()) {
    ...
    if (rf95.recv(buf, &len)) {
      ...
      Serial.print("RF95 From: ");
      Serial.println(rf95.headerFrom());
      Serial.print("RF95 To: ");
      Serial.println(rf95.headerTo());
      digitalWrite(led, LOW);

      // Read app headers here as well
//      uint16_t from_net = ntohs(net);
//      memcpy(p, &from_net, sizeof(from_net));
//      p += sizeof(from_net);
...
void sendACK(char* ACKMessage, int messageSize) {
  if (rf95.waitCAD()) {
    ...
    rf95.send((const uint8_t*)ACKMessage, messageSize);
    rf95.waitPacketSent();
...
void sendMessage(char* message) {
  ...
  memcpy(p, &h.last_rssi, sizeof(h.last_rssi));
